...
The function $ f (x) = \sin(x) $ is smooth real analytic and its Taylor expansion is as follows
...
\section{Diffeomorphisms between open sets of $ \R^{n} $}

Let $ U, V \in \R^{n} $ be opens sets, we say that the function $ f : U \to V $ is a \textit{diffeomorphism} if it is a bijection\footnote{%
	Therefore invertible.%
}, $ f \in C^{\infty}(U) $ and its inverse $ g : V \to U $ is $ g \in C^{\infty}(V) $. \\
...
is a smooth bijection but its inverse is not smooth, therefore $ f $ is not a diffeomorphism. \\
When there exists a diffeomorphism between two open sets, we say that they are \textit{diffeomorphic}, i.e. $ U $ and $ V $ are diffeomorphic if exists a diffeomorphism $ f : U \to V $, notation-wise $ U \simeq V $. \\
