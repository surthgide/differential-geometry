...
Dai risultati precedenti, dato un gruppo di Lie $ G $, possiamo costruire un'algebra di Lie $ \g = (T_{e}(G),[,]) $ e da questo possiamo tornare indietro tramite l'esponenziale $ \exp $: questo dimostra che esiste un funtore $ \mathcal{F} $ dalla categoria dei gruppi di Lie $ \mathfrak{G} $ alla categoria delle algebre di Lie $ \mathfrak{A} $ che porta omomorfismi di gruppi di Lie $ F : H \to G $ in omomorfismi di algebre di Lie $ F_{*e} : \h \to \g $

\begin{equation}
	\begin{cases}
		\mathcal{F}(G) = T_{e}(G) & \forall G \in Ob(\mathfrak{G}), \, T_{e}(G) \in Ob(\mathfrak{A})\\
		\mathcal{F}(F) = F_{*p} & \forall F \in Mor(H,G), \, F_{*e} \in Mor(\h,\g)
	\end{cases}
\end{equation}
...
\begin{theorem}
	...
	\begin{equation}
		g = \prod_{j=1}^{k} \exp(\xi_{j}) \qcomma \forall g \in G, \, \xi_{j} \in T_{e}(G), \, j=1,\dots,k
	\end{equation}
