...
\begin{proof}
	...
	Questo rende il differenziale in un punto qualsiasi $ A \in SU(2) $ un isomorfismo in quanto composizione di isomorfismi: per il teorema della funzione inversa\footnote{%
		Vedi Teorema \ref{ift}.%
	}, $ R $ sarà un diffeomorfismo locale e dunque una sommersione (in quanto il suo differenziale è suriettivo poiché isomorfismo) da cui, per il corollario del teorema di sommersione locale\footnote{%
		Vedi Corollario \ref{th-somm-loc-cor}.%
	}, abbiamo che $ R $ è un'applicazione aperta. Questa applicazione però è anche chiusa per il lemma dell'applicazione chiusa\footnote{%
