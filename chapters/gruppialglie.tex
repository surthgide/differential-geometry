...
L'immagine di $ \R $ tramite $ F $ è un sottogruppo di $ \T^{2} $, \ie $ F(\R) < \T^{2} $, è una sottovarietà immersa di $ \T^{2} $ ($ F $ non è un embedding) e le operazioni su $ F(\R) $ sono lisce, dunque $ F(\R) $ è un sottogruppo di Lie (immerso) di $ \T^{2} $.
...
\begin{itemize}
	...
	\item la matrice è un multiplo della matrice identità $ X = x I_{n} $, \ie $ X^{i} = x^{i} I_{n} $, da cui $ e^{X} = e^{x} I_{n} $.
\end{itemize}
...
\begin{definition}
	...
	\begin{itemize}
		\item Se $ s_{n} \in V $ è una successione convergente, \ie $ s_{n} \to s \in V $, allora
