...
perché $ f \circ g = \operatorname{id}_{\mathbb{R}^{n}} \, \wedge \, g \circ f = \operatorname{id}_{B_{1} (0)} $.\\
...
Consideriamo la semicalotta aperta in $ \mathbb{R}^{n+1} $ centrata in $ (0,0,1) $ di raggio $ 1 $:

\begin{equation}
	S = \left\{ (x^{1}, \dots, x^{n+1}) \in \mathbb{R}^{n+1} \middle| (x^{n+1})^{2} + \sum_{i=1}^{n} (x^{i})^{2} = 1 \, \wedge \, x^{n+1} < 1 \right\}
\end{equation}
...
Siano $ f : U \to \mathbb{R} $ con $ f \in C^{\infty}(\mathbb{R}) $, $ p\in U $ e $ v \in T_{p}(\mathbb{R}^{n}) $. Consideriamo la retta $ c(t) $ che passa per $ p $ con direzione $ v $, parametrizzata come $ c(t)=p+tv $ con $ t \in \mathbb{R} $. Definisco la \textit{derivata direzionale} di $ f $ rispetto a $ v $ come
...
Un'algebra $ A $ su un campo $ \mathbb{K} $ è una coppia $ (V,\cdot) $ con $ V $ spazio vettoriale su $ \mathbb{K} $ (dunque con operazioni $ a+b \in A $ con $ a,b \in A $ e $ \lambda a \in A $ con $ \lambda \in \mathbb{K} $) e un'operazione binaria
...
per $ \forall a,b,c \in A $ e $ \forall \lambda \in \mathbb{K} $.\\
...
Nonostante non sia necessario per un'algebra, $ C_{p}^{\infty}(\mathbb{R}^{n}) $ è anche commutativa e unitaria\footnote{%
...
\subsection{Isomorfismo tra $ T_{p}(\mathbb{R}^{n}) $ e $ Der_{p}(C_{p}^{\infty}(\mathbb{R}^{n})) $}
...
Date queste proprietà di $ \phi $, questa applicazione è un omeomorfismo tra $ T_{p}(\mathbb{R}^{n}) $ e $ Der_{p}(C_{p}^{\infty}(\mathbb{R}^{n})) $, i.e.
...
\section{Campi di vettori su aperti di $ \mathbb{R}^{n} $}
...
\subsubsection{$ \mathbb{R} $-modulo destro}
...
} su un campo $ \mathbb{K} $, un'applicazione $ D : A \to A $ che sia $ \K $-lineare e tale che soddisfi la regola di Leibniz
...
Tramite questo isomorfismo, si possono identificare i campi di vettori lisci con le derivazioni dell'algebra delle funzioni lisce, analogamente a come lo spazio tangente ad un punto di $ \mathbb{R}^{n} $ si può identificare con le derivazioni puntuali dell'algebra dei germi delle funzioni in quel punto, i.e. $ T_{p}(\mathbb{R}^{n}) \simeq Der_{p}(C_{p}^{\infty}(\mathbb{R}^{n})) $.