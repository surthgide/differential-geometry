...
e analogamente per le altre composizioni. Tutte queste applicazioni e le loro inverse sono $ C^{\infty} $ per lo stesso motivo dato per il cerchio unitario.\\
...
Per dotare $ \S^{n} $ di una struttura differenziabile prendendo l'atlante massimale che include queste due carte, è necessario verificare che il cambio di carte sia $ C^{\infty} $
...
e la sua inversa è identica a sé stessa $ \pi_{S} \circ \pi_{N}^{-1} \equiv \pi_{N} \circ \pi_{S}^{-1} $; queste composizioni sono $ C^{\infty} $ dunque le carte $ (U_{N},\pi_{N}) $ e $ (U_{S},\pi_{S}) $ sono $ C^{\infty} $-compatibili perciò $ \{(U_{N},\pi_{N}),(U_{S},\pi_{S})\} $ è un atlante differenziabile su $ \S^{n} $, il quale diventa una varietà differenziabile.\\
...
dove $ \dim M = m $ e $ \dim N = n $, sono omeomorfismi perché continue (il prodotto di applicazioni continue è continuo) e le inverse sono il prodotto delle inverse
...
Il toro $ \T^{2} = \S^{1} \times \S^{1} $ e il cilindro infinito $ \C = \S^{1} \times \R $ sono superfici differenziabili; anche il toro $ n $-dimensionale è una varietà differenziabile

\begin{align}
	\T^{n} = \prod^{n} \S^{1}
\end{align}

dove $ \prod $ implica la topologia prodotto $ \times $.
...
Definiamo l'applicazione $ f : S \to Y $ con $ S $ e $ Y $ spazi topologici: questa applicazione è \textit{costante sulle classi di equivalenza} cioè, presi due elementi $ s_{1},s_{2} \in S $
...
L'applicazione $ \pi : \R \to \sfrac{\R}{\{-1,1\}} $ non è aperta perché, per esempio $ \pi((0,2)) $ non è aperto in $ \sfrac{\R}{\{-1,1\}} $ perché la sua controimmagine $ \pi^{-1}(\pi((0,2))) = (0,2) \cup \{-1\} $ in quanto i punti $ \{-1,1\} $ sono equivalenti, ma questa controimmagine non è aperta in $ \R $.
...
\begin{proof}[Dimostrazione (Proposizione quoziente $ T_{2} $)]\label{solved-es2-3}
...
\begin{equation}
	x \sim y \, \iff \, \exists \lambda \in \R \setminus \{0\} \, \mid \, y = \lambda x
\end{equation}

cioè se appartengono alla stessa retta.\\
...
\begin{definition}
	...
	\begin{equation}
		\left. \mathcal{RP}^{n} \simeq \sfrac{\S^{n}}{\sim_{a}} \quad \middle| \quad x \sim_{a} y \iff y = \pm x \right.
	\end{equation}
\end{definition}
...
\begin{proof}[Dimostrazione (Omeomorfismo $ \mathcal{RP}^{n} \simeq \sfrac{\S^{n}}{\sim_{a}} $)]
...
in quanto questa applicazione preservi la proprietà di numerabilità $ N_{2} $: se lo spazio di partenza $ \R^{n+1} \setminus \{0\} $ è $ N_{2} $ (lo è perché sottoinsieme di $ \R^{n+1} $ il quale è a sua volta $ N_{2} $) allora, se l'applicazione $ \pi $ è aperta (equivalentemente, se $ \sim $ è aperta), lo è anche il proiettivo reale.\\
...
Se lo spazio di partenza $ \R \setminus \{0\} $ è $ N_{2} $ e la relazione di equivalenza $ \sim $ è aperta, allora il quoziente $ \mathcal{RP}^{n} $ è $ N_{2} $.\\
...
\begin{align}
	\begin{split}
		\phi : \R^{n+1} \setminus \{0\} \times \R^{n+1} \setminus \{0\} &\to M_{n+1,2}(\R) = \R^{2(n+1)}\\
		(x,y) &\mapsto \begin{bmatrix} x & y \end{bmatrix}
	\end{split}
\end{align}

cioè che associa alle coppie di vettori $ (x,y) $ le matrici $ (n+1) \times 2 $ costituite dai vettori $ x $ e $ y $ posti in colonne affiancate, i.e.
...
\begin{equation}
	\phi(R) = \left\{ \begin{bmatrix} x & y \end{bmatrix} \in M_{n+1,2}(\R) \, \middle| \, \rank (\begin{bmatrix} x & y \end{bmatrix}) = 1 \right\}
\end{equation}
...
\begin{equation}
	\phi(R) = X_{1} \cap \phi(\R^{n+1} \setminus \{0\} \times \R^{n+1} \setminus \{0\})
\end{equation}
...
\begin{equation}
	V_{i} = \left\{ (a^{0},\dots,a^{n}) \in \R^{n+1} \setminus \{0\} \, \middle| \, a^{i} \neq 0, \, i=0,\dots,n \right\} \subset \R^{n+1} \setminus \{0\}
\end{equation}
...
Sia $ k \leqslant n $, definisco la \textit{grassmanniana} $ G(k,n) $ come l'insieme di tutti i sottospazi vettoriali di $ \R^{n} $ di dimensione $ k $.\\
...
\begin{equation}
	A \sim B \, \iff \, \exists g \in GL_{k}(\R) \, \mid \, B = A g
\end{equation}
...
dove $ V g = \{ A g \, \mid \, g \in GL_{k}(\R) \, \wedge \, A \in V \} $, prendiamo l'applicazione
...
\begin{equation}
	R = \left\{ (A,B) \in F(k,n) \times F(k,n) \, \middle| \, B = A g, \, g \in GL_{k}(\R) \right\}
\end{equation}
...
\begin{align}
	\begin{split}
		\phi : M_{n,k}(\R) \times M_{n,k}(\R) &\to M_{n,2k}(\R)\\
...
Preso $ B = A g $ (cioè $ A \sim B $), abbiamo che $ A_{ij} $ è invertibile se e solo se $ B_{ij} $ è invertibile
...
\begin{align}
	\begin{split}
		...
		&= \left[ \begin{pmatrix} \id_{2 \times 2} \\ A_{34} (A_{12})^{-1} \end{pmatrix} \right]\\
...
\begin{align}
	\begin{split}
		\phi_{13} \circ (\phi_{12})^{-1} : \R^{4} &\to \R^{4}\\
...
Sia $ M $ una varietà differenziabile di dimensione $ m $ e sia la funzione $ f : M \to \R $, diremo che $ f $ è $ C^{\infty} $ (liscia o differenziabile) in un punto $ p \in M $ se esiste una carta $ (U,\phi) $ intorno a $ p $ tale che la funzione $ f \circ \phi^{-1} : \phi(U) \to \R $, con $ \phi(U) \subset \R^{n} $, sia $ C^{\infty} $.
...
Una funzione $ f : M \to \R $ è $ C^{\infty} $ se lo è in ogni punto.

\begin{remark}
	Se $ f : M \to \R \in C^{\infty} $ allora $ f $ è continua. Infatti $ f_{|\phi(U)} = f \circ \phi^{-1} \circ \phi $ dove $ \phi $ è continua perché omeomorfismo e $ f \circ \phi^{-1} $ è continua in quanto $ C^{\infty} $.
\end{remark}

Indicheremo $ C^{\infty}(M) $ l'insieme delle funzioni $ f : M \to \R \in C^{\infty} $ su $ M $ mentre $ C^{0}(M) $ sono le funzioni continue.

\begin{definition}
	Sia $ M $ una varietà differenziabile con atlante massimale $ \mathfrak{M} $ e sia un atlante differenziabile $ \mathfrak{U} \subset \mathfrak{M} $, allora $ f : M \to \R $ è $ C^{\infty} $ se e solo se per qualunque carta $ (V,\psi) \in \mathfrak{U} $ si ha che $ f \circ \psi^{-1} : \psi(V) \to \R $ è $ C^{\infty} $.
\end{definition}

\begin{proof}[Dimostrazione ($ \implies $)]
	Se $ f \in C^{\infty}(M) $ allora esiste una carta $ (V,\psi) $ dell'atlante massimale tale che $ f \circ \psi : \psi(V) \to \R $ sia $ C^{\infty} $ in $ p $ allora lo è anche in $ \psi(V) $ in quanto la scelta non dipende dall'atlante, quindi possiamo prendere la carta dell'atlante $ \mathfrak{U} $.
\end{proof}

\begin{proof}[Dimostrazione ($ \Longleftarrow $)]
	Sia $ p \in M $, allora esiste una carta $ (V,\psi) \ni p $ tale che, per ipotesi, $ f \circ \psi : \psi(V) \to \R $ sia $ C^{\infty} $, dunque abbiamo trovato una carta in cui è liscia perciò $ f \in C^{\infty} $ in $ p $.
\end{proof}

Riassumendo, presa una varietà differenziabile $ M $ e un atlante differenziabile arbitrario $ \mathfrak{U} \subset \mathfrak{M} $ contenuto nella struttura differenziabile, allora $ f : M \to \R $ è $ C^{\infty} $ se e solo se per $ \forall (U,\phi) \in \mathfrak{U} $ abbiamo che $ f \circ \phi^{-1} : \phi(U) \to \R $ sia $ C^{\infty} $.
...
con $ \in \R^{n} $, sia $ C^{\infty} $ in $ \phi(p) $\footnote{%
...
\begin{remark}
	Nel caso in cui lo spazio ambiente sia tutto $ \R^{n} $, i.e. $ F : N \to \R^{n} $, dove la struttura differenziale dello spazio euclideo $ \R^{n} $ è data dall'atlante massimale che contiene $ (\R^{n},\id_{\R^{n}}) $, la funzione $ F $ è $ C^{\infty} $ in $ p $ se e solo se l'applicazione
...
\begin{definition}
	...
	\begin{equation}
		\psi' \circ F \circ (\phi')^{-1} : \phi'(F^{-1}(V) \cap U') \to \R^{m}
	\end{equation}
...
\begin{definition}[Componenti]
	Sia $ F : N \to \R^{n} $ un'applicazione e sia la proiezione
...
\begin{proof}[Dimostrazione ($ \Longleftarrow $)]
	Supponiamo che $ r^{i} \circ F \doteq F^{i} : N \to \R $ sia $ C^{\infty} $ per $ \forall i=1,\dots,n $. Per $ \forall p \in N $ abbiamo che $ F^{i} \circ \phi^{-1} : \phi(U) \to \R $ è $ C^{\infty} $ se scegliamo una carta $ (U,\phi) \ni p $, dove $ \phi(p) \subset \R^{n} $. A questo punto, la funzione
	...
	è $ C^{\infty} $ in $ \phi(p) $, dunque $ F $ è $ C^{\infty} $ in $ p $ per definizione, perché $ F \circ \phi^{-1} \equiv \id_{\R^{n}} \circ F \circ \phi^{-1} $.
\end{proof}

Riassumendo, prese due varietà differenziabili $ N $ ed $ M $ e due atlanti differenziabili arbitrari $ \mathfrak{U} \subset \mathfrak{N} $ e $ \mathfrak{V} \subset \mathfrak{M} $ contenuti nelle rispettive strutture differenziabili, allora $ F : N \to M $ è $ C^{\infty} $ se e solo se $ F $ è continua e per $ \forall (U,\phi) \in \mathfrak{U} $ e $ \forall (V,\psi) \in \mathfrak{V} $ abbiamo che $ \psi \circ F \circ \phi^{-1} : \phi(F^{-1}(V) \cap U) \to \R^{m} $ sia $ C^{\infty} $.
...
Fissiamo gli atlanti $ (\R,\id_{\R}) $ e $ (U_{i},\phi_{i}) $ con $ i=1,\dots,4 $ dove gli $ U_{i} $ sono le varie semicirconferenze di $ \S^{1} $ e le $ \phi_{i} $ sono le proiezioni di queste sugli assi\footnote{%
...
Dobbiamo verificare che l'applicazione $ \phi_{i} \circ F \circ \id_{\R} $ sia $ C^{\infty} $ per $ \forall i=1,\dots,4 $: innanzitutto $ \phi(F^{-1}(V) \cap U) $ della definizione corrisponde a $ \id_{\R}(F^{-1}(U_{i}) \cap \R) = F^{-1}(U_{i}) \subset \R $, dunque
...
le quali sono tutte $ C^{\infty} $ perciò lo è anche $ F $.
...
\begin{theorem}
	Due varietà differenziabili di dimensione 1 connesse (curve differenziabili) sono diffeomorfe o a $ \S^{1} $ (compatta) o a $ \R $ (non compatta).
\end{theorem}
...
Queste due strutture differenziali sono diverse, cioè le loro carte non sono $ C^{\infty} $-compatibili, infatti il cambio di carta $ \psi \circ (\id_{\R})^{-1} = \psi $ non è $ C^{\infty} $, però sono diffeomorfe tramite l'applicazione
...
L'identità $ \id $ da $ \R $ in $ \R' $ non è un diffeomorfismo in quanto
...
\begin{proof}
	Osserviamo che $ (U,\phi) $ è un'atlante differenziabile per $ U $ e $ (\phi(U),\id_{\R^{n}}) $ è un atlante differenziabile per $ \phi(U) $, allora per la definizione di funzione $ C^{\infty} $ tra varietà
...
\begin{align}
	\begin{split}
		\phi_{\alpha} \circ \pi_{N} \circ (\phi_{\alpha} \times \psi_{\beta})^{-1} : \phi_{\alpha}(U_{\alpha}) \times \psi_{\beta}(V_{\beta}) &\to \R^{m}\\
...
dove $ F_{1} : N \to M_{1} $ e $ F_{1} : N \to M_{2} $, allora $ F $ è $ C^{\infty} $ se e solo se lo sono $ F_{1} $ e $ F_{2} $.\\
...
Il gruppo $ (\R \setminus \{0\},\cdot) $ è una varietà differenziabile e gruppo di Lie in quanto
...
Sia $ F^{i} = y^{i} \circ F = r^{i} \circ \psi \circ F $ con $ i=1,\dots,m $, dove $ \psi = (y^{1},\dots,y^{m}) $, la componente $ i $-esima di $ F $ rispetto alle coordinate $ (y^{1},\dots,y^{m}) $. L'applicazione $ F^{i} : U \to \R $ è $ C^{\infty}(U) $, quindi possiamo definire le sue derivate parziali
...
Possiamo quindi calcolare la sua matrice jacobiana, sapendo che le $ y^{i} : U \cap V \to \R $ sono $ C^{\infty} $
...
\begin{proof}
	...
	Il fatto che $ \psi \circ F \circ \phi^{-1} $ sia un diffeomorfismo locale in $ \phi(p) $ significa che esiste un aperto $ W \ni p $ tale che $ \psi \circ F \circ \phi^{-1} : W \to \psi(F(\phi^{-1}(W))) $ sia un diffeomorfismo, in cui sia $ W $ che $ \psi(F(\phi^{-1}(W))) $ sono aperti di $ \R^{n} $. Se questo è un diffeomorfismo, allora lo è anche $ \psi^{-1} \circ \psi \circ F \circ \phi^{-1} \circ \phi = F $ in un intorno di $ p $, i.e. $ \phi^{-1}(W) $.
\end{proof}
...
Negli aperti di $ \R^{n} $ possiamo definire lo spazio tangente in un punto come vettore di $ n $ componenti uscente in quel punto oppure se $ U \subset \R^{n} $ allora lo spazio tangente $ T_{p}(U) = T_{p}(\R^{n}) \simeq Der_{p}(C_{p}^{\infty}(\R^{n})) $ dunque può essere pensato come lo spazio delle derivazioni puntuali dell'algebra dei germi delle funzioni $ C^{\infty} $, dove $ C_{p}^{\infty}(U) \equiv C_{p}^{\infty}(\R^{n}) $ in quanto due funzioni sono equivalenti in un punto se coincidono in un aperto, anche piccolo, che contiene il punto stesso. Quest'ultimo approccio, siccome non è possibile avere una visualizzazione nelle varietà, si presta maggiormente per definire lo spazio tangente su una varietà.\\
...
Come per $ \R^{n} $, $ C_{p}^{\infty}(N) $ è ancora un'algebra su $ \R $ con operazioni di somma e prodotto per scalare (per far sì che sia uno spazio vettoriale) e tra funzioni
...
inoltre è $ \R $-lineare e soddisfa la regola di Leibniz.\\
...
Per mostrare che $ F_{*p} : T_{p}(N) \to T_{F(p)}(M) $ sia un'applicazione lineare, prendiamo due vettori $ X_{p},Y_{p} \in T_{p}(N) $,due numeri reali $ \alpha,\beta \in \R $ e $ f \in [(f,U)] \in C_{F(p)}^{\infty}(M) $
...
per $ j=1,\dots,n $, in quanto $ F_{*p} $ è lineare. Le incognite sono le $ a_{kj} $ e, per trovarle, applichiamo entrambi i membri dell'equazione allo stesso oggetto, i.e. la proiezione naturale in $ \R^{m} $
...
\paragraph{4. Proprietà funtoriali}
...
Sia un diffeomorfismo $ F : N \to M $, allora $ F_{*p} : T_{p}(N) \to T_{F(p)}(M) $ è un isomorfismo tra spazi vettoriali: essendo $ F $ un diffeomorfismo, esiste una funzione $ G : M \to N $ per cui $ G \circ F = \id_{N} \, \wedge \, F \circ G = \id_{M} $ dunque
...
\begin{theorem}
	Sia $ F : U \to V $ un diffeomorfismo dove $ U \subset \R^{n} $ e $ V \subset \R^{m} $ sono aperti con $ \dim(N)=n $ e $ \dim(M)=m $, allora $ n=m $.
\end{theorem}
...
\begin{proof}
	Essendo $ F : U \to V $ un diffeomorfismo, sappiamo che $ F_{*p} : T_{p}(U) = T_{p}(\R^{n}) \to T_{F(p)}(V) = T_{F(p)}(\R^{m}) $ è un isomorfismo, ma un isomorfismo tra due spazi di dimensione finita può esistere solo se i due spazi hanno la stessa dimensione, il che implica $ \dim(T_{p}(\R^{n})) = \dim(T_{F(p)}(\R^{m})) $ per cui $ n=m $.
\end{proof}
...
Una \textit{curva} liscia $ c $ su una varietà differenziabile $ N $ è un'applicazione $ C^{\infty} $  tale che $ c : (a,b) \subset \R \to N $. Se $ 0 \in (a,b) $ diremo che $ c $ \textit{inizia} in $ c(0)=p $.
...
\begin{lemma}
	...
	\begin{equation}
		\forall \varepsilon > 0, \, \exists k \in \Z \setminus 0 \mid \left| (e^{2 \pi i k}, e^{2 \pi i \alpha k}) - (1,1) \right| < \varepsilon
	\end{equation}
\end{lemma}
