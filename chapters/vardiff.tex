...
\begin{theorem}[Rango costante in analisi]
	...
	\begin{equation}
		\rank(J(F)(q)) = \rank \left[ \pdv{F^{i}}{x^{j}} \, (q) \right] = k \leqslant \min\{n,m\} \qcomma \forall q \in I_{1}
	\end{equation}
...
\begin{theorem}[Rango costante in geometria differenziale]
	...
	\begin{equation}
		\begin{cases}
			(U,\phi) \in N \qcomma \phi(p) = 0 \\
			(V,\psi) \in M \qcomma \psi(F(p)) = 0 \\
			F(U) \subseteq V
		\end{cases}
	\end{equation}
...
\begin{proof}
	...
	\begin{equation}
		\begin{cases}
			(\tilde{U},\tilde{\phi}) \in N \qcomma \tilde{\phi}(p) = 0 \\
			(\tilde{V},\tilde{\psi}) \in M \qcomma \tilde{\psi}(F(p)) = 0 \\
	...
	\begin{equation}
		\rank(\tilde{\psi} \circ F \circ \tilde{\phi}^{-1})(\tilde{\phi}(q)) = k \qcomma \forall q \in \tilde{U}
	\end{equation}
...
\begin{theorem}[Preimmagine di un'applicazione di rango costante]
	...
	Allora $ F^{-1}(c) $ è una sottovarietà di $ N $ con $ \dim(F^{-1}(c)) = n-k $ o, equivalentemente, $ \operatorname{cod}_{N}(F^{-1}(c)) = k $.
\end{theorem}
