...
\begin{equation}
	(\sin(2t_{1}),\sin(t_{1})) = (\sin(2t_{2}),\sin(2t_{2})) %
	\implies %
	\begin{cases}
		\sin(t_{1}) = \sin(t_{2}) = 0 \\
		\sin(t_{1}) = \sin(t_{2}) \neq 0
...
\begin{equation}
	t_{1} = t_{2} = 0 \, \because \, t \in (-\pi,\pi)
\end{equation}
...
dove $ \alpha \in \R \setminus \Q $ (i.e. $ \alpha $ irrazionale); siccome il toro $ \T^{2} $ è incluso in $ \R^{4} $, possiamo riscrivere la funzione come
