...
Tramite il seguente diagramma

\begin{figure}[H]
	\centering
	\begin{tikzcd}
		S \arrow[r, "f"] \arrow[d, "\pi"']        & Y \\
		\sfrac{S}{\sim} \arrow[ru, "\tilde{f}"'] &  %
	\end{tikzcd}
\end{figure}

possiamo derivare la seguente proprietà
...
\begin{figure}[H]
	\centering
	\begin{tikzcd}
		I \arrow[r, "f"] \arrow[d, "\pi"']        & \S^{1} \\
		{\sfrac{I}{0,1}} \arrow[ru, "\tilde{f}"'] &       
	\end{tikzcd}
\end{figure}
...
Se prendiamo $ n=1 $, $ \rp{1} \simeq \sfrac{\S^{1}}{\sim_{a}} $: si può identificare $ \sfrac{\S^{1}}{\sim_{a}} $ con l'intervallo $ I $ (omeomorfo alla semicirconferenza) quozientato i due punti estremi $ \{A,B\} $, i.e. $ \S^{1} \simeq \sfrac{I}{\{A,B\}} $, in modo tale che si prenda solo uno dei due punti antipodali.
...
Se lo spazio di partenza $ \R \setminus \{0\} $ è $ N_{2} $ e la relazione di equivalenza $ \sim $ è aperta, allora lo spazio quoziente $ \rp{n} $ è $ N_{2} $.
...
A questo punto, dimostrando che l'immagine di $ R $ tramite $ \phi $ è un chiuso, otterremo che il proiettivo reale è di Hausdorff. L'insieme in questione è il seguente:s

\begin{equation}
	\phi(R) = \left\{ \mqty[x & y] \in M_{n+1,2}(\R) \st \rank \left( \mqty[x & y] \right) = 1 \right\}
\end{equation}

La condizione per cui il rango delle matrici di questo insieme è unitario è dovuta al fatto che queste matrici hanno le due colonne proporzionali tra loro, in quanto $ y= \lambda x $.

\begin{lemma}\label{lemma-clos-matrix}
	Sia l'insieme
	
	\begin{equation}
		X_{r}(n,m) = \left\{ A \in M_{n,m}(\R) \st \rank(A) \leqslant r \, \wedge \, r \leqslant \min\{n,m\} \right\}
	\end{equation}
...
\begin{proof}
	...
	} di minori $ m_{ij} \doteq \det(A_{ij}) $ di $ A $ di ordine $ r < \min\{n,m\} $, dove le $ A_{ij} $ sono le sottomatrici ricavate da $ A $ rimuovendo la $ i $-esima riga e la $ j $-esima colonna. Questa applicazione è continua perché è composizione di due applicazioni continue: l'applicazione che rimuove righe e colonne alla matrice $ A $ e il determinante di questa stessa matrice.\\
...
Seguendo questo ragionamento, possiamo pensare a $ \phi(R) $ come l'intersezione dell'immagine dello spazio di Hausdorff $ \R^{n+1} \setminus \{0\} \times \R^{n+1} \setminus \{0\} $ tramite $ \phi $ con $ X_{1}(n+1,2) $, il quale è un chiuso, i.e.
...
Gli insiemi $ U_{i} $ sono aperti in $ \rp{n} $ perché $ \pi $ è aperta e i $ V_{i} $ sono aperti in $ \R^{n+1} \setminus \{0\} $.\\
Questi insiemi sono un ricoprimento per $ \rp{n} $
...
Essendo l'unione degli insiemi $ U_{i} $ un ricoprimento per $ \rp{n} $, dimostrando che le $ \phi_{i} $ sono degli omeomorfismi, facciamo vedere che $ \mathfrak{M} = \{(U_{i},\phi_{i})\} $ è un atlante topologico per $ \rp{n} $.\\
...
siccome è presente la componente $ i $-esima uguale a 1, la classe $ [(x^{1},\dots,x^{i-1},1,x^{i},x^{i+1},\dots,x^{n})] $ appartiene a $ \rp{n} $ anche se $ (x^{1},\dots,x^{n}) = (0,\dots,0) $. Queste applicazioni sono le inverse di $ \phi_{i} $ perché
...
e sono continue. Dunque abbiamo dimostrato che le $ \phi_{i} $ sono omeomorfismi e quindi $ \mathfrak{M} = \{(U_{i},\phi_{i})\}_{i=0,\dots,n} $ è un atlante topologico per $ \rp{n} $.\\
Dimostriamo ora che $ \mathfrak{M} $ è un atlante differenziabile di $ \rp{n} $, cioè dobbiamo far vedere che le sue carte siano $ C^{\infty} $-compatibili tra loro: siano $ i,j=0,\dots,n $ con $ i<j $ e prendiamo i cambi di carta
...
entrambi i cambi di carta sono lisci quindi $ \mathfrak{M} $ è un atlante differenziabile: questo dimostra l'esistenza di un atlante differenziabile massimale che rende $ \rp{n} $ una varietà differenziabile.
...
Essendo $ \rp{n} $ l'insieme di tutti i sottospazi vettoriali di dimensione 1 (i.e. rette) di $ \R^{n+1} $, si ha che
...
\begin{equation}
	M_{n,k}(\R) \setminus F(k,n) = \left\{ A \in M_{n,k}(\R) \st \rank(A) \leqslant k \right\}
\end{equation}
...
con la struttura differenziale è ereditata da $ M_{n}(\R) = \R^{n^{2}} $, in quanto $ GL_{n}(\R) $ è aperto in questo spazio, è un gruppo di Lie rispetto alla moltiplicazione. La carta $ (GL_{n}(\R),\id) $ corrisponde a un atlante per $ GL_{n}(\R) $.\\
...
Fissiamo una varietà differenziabile $ N $ di dimensione $ n $ e una carta $ (U,\phi) $ intorno a un punto $ p $, dunque abbiamo il diffeomorfismo $ \phi : U \to \phi(U) \subset \R^{n} $. Prendendo la proiezione
...
Consideriamo una funzione $ f : N \to \R $ che sia $ C^{\infty}(N) $, vogliamo definire la \textit{derivata parziale di} $ f $ \textit{rispetto alla coordinata} $ x^{i} $, i.e. $ \sfrac{\partial f}{\partial x^{i}} (p) $, la quale dipenderà dunque dalla carta scelta. L'idea alla base della derivata parziale di una funzione su una varietà è quella di riportare tutto a $ \R^{n} $ e svolgere la derivata usuale, dunque definiamo
