...
%
Sia $ \mathfrak{U} = \{(U_{\alpha},\varphi_{\alpha})\} $ un atlante topologico di uno spazio $ M $, diremo che $ (V,\psi) $ carta di $ M $ è $ C^{\infty} $-compatibile con $ \mathfrak{U} $ se $ (V,\psi) $ è $ C^{\infty} $-compatibile con $ (U_{\alpha},\varphi_{\alpha}) $ per $ \forall \alpha \in A $.
...
Un atlante topologico $ \mathfrak{U} = \{(U_{\alpha},\varphi_{\alpha})\} $ è detto \textit{differenziabile} (liscio o $ C^{\infty} $) se ogni sua carta è $ C^{\infty} $-compatibile con ogni altra carta al suo interno. \\
Diremo che un atlante differenziabile $ \mathfrak{M} = \{(U_{\alpha},\varphi_{\alpha})\} $ è \textit{massimale} se per qualunque altro atlante differenziabile $ \mathfrak{M}' $ di $ M $ si ha che
...
Infatti, se prendiamo l'atlante massimale differenziabile $ \mathfrak{M} = \{(U_{\alpha},\varphi_{\alpha})\} $ e consideriamo $ \varphi \circ \varphi_{\alpha}^{-1} $
...
In particolare, se $ U $ è un aperto qualunque di $ M $ e $ (U_{\beta},\varphi_{\beta}) $ è una carta della struttura differenziabile, allora $ (U \cap U_{\beta}, \eval{\varphi_{\beta}}_{U \cap U_{\beta}}) $ è una carta nella struttura differenziabile.
...
Per le superfici differenziabili, $ \dim (M) = 2 $. Le \textit{parametrizzazioni} $ X $ di curve e superfici sono l'inverso delle carte $ \varphi = X^{-1} $, i.e per la parametrizzazione di una superficie
...
Dimostriamo prima che le $ \varphi_{i} $ siano omeomorfismi: sono continue e lisce perché sono restrizioni a una delle componenti e le inverse
...
\sbs{0.45}{%
		...
		{0.52}{%
			Ad esempio, l'applicazione $ \varphi_{1} $ è rappresentata di seguente:
			\img{1}{img9}
		}
...
\begin{equation}
	\varphi(R) = \left\{ \pmqty{x & y} \in M_{n+1,2}(\R) \st \rank \left( \pmqty{x & y} \right) = 1 \right\}
\end{equation}
...
\begin{gather}
	(\varphi_{12} \circ (\varphi_{12})^{-1})(A) = \varphi_{12} \left( \pmqty{ \bigone_{2 \times 2} \\ A } \right) = A (\bigone_{2 \times 2})^{-1} = A \\ \\
	%
	((\varphi_{12})^{-1} \circ \varphi_{12})([A]) = (\varphi_{12})^{-1}(A_{34} (A_{12})^{-1}) = \left[ \pmqty{ \bigone_{2 \times 2} \\ A_{34} (A_{12})^{-1} } \right] = \left[ \pmqty{ A_{12} \\ A_{34} } \right] = [A]
\end{gather}
...
\begin{align}
	\begin{split}
		(\varphi_{13} \circ (\varphi_{12})^{-1}) (C) &= (\varphi_{13} \circ (\varphi_{12})^{-1}) \left( \pmqty{ c_{11} & c_{12} \\ c_{21} & c_{22} } \right) \\ \\
		&= \varphi_{13} \left( \left[ \pmqty{ 1 & 0 \\ 0 & 1 \\ c_{11} & c_{12} \\ c_{21} & c_{22} } \right] \right) \\ \\
...
con $ \varphi(F^{-1}(V) \cap U) \subset \R^{n} $, sia liscia in $ \varphi(p) $. Possiamo scrivere $ \eval{F}_{U} \equiv F $ in quanto agisce prima $ \varphi^{-1} $ che restringe il dominio all'aperto $ U $. \\
...
Riassumendo, prese due varietà differenziabili $ N $ ed $ M $ e due atlanti differenziabili arbitrari $ \mathfrak{U} \subset \mathfrak{N} $ e $ \mathfrak{V} \subset \mathfrak{M} $ contenuti nelle rispettive strutture differenziabili, abbiamo che un'applicazione $ F : N \to M $ è liscia se e solo se è continua e $ \psi \circ F \circ \varphi^{-1} : \varphi(F^{-1}(V) \cap U) \to \R^{m} $ è liscia per $ \forall (U,\varphi) \in \mathfrak{U} $ e $ \forall (V,\psi) \in \mathfrak{V} $.
...
\begin{proof}[Dimostrazione ($ \impliedby $)]
	Supponiamo che $ r^{i} \circ F \doteq F^{i} : N \to \R $ sia liscia per $ \forall i=1,\dots,n $. Per $ \forall p \in N $ abbiamo che $ F^{i} \circ \varphi^{-1} : \varphi(U) \to \R $ è liscia se scegliamo una carta $ (U,\varphi) \ni p $, dove $ \varphi(p) \subset \R^{n} $. A questo punto, la funzione
	
	\map{F \circ \varphi^{-1}}
		{\varphi(U)}{\R^{n}}
		{q}{(F^{1} \circ \varphi^{-1}, \dots, F^{n} \circ \varphi^{-1})(q)}

	è liscia in $ \varphi(p) $, dunque $ F $ è liscia in $ p $ per definizione, perché $ F \circ \varphi^{-1} \equiv \id_{\R^{n}} \circ F \circ \varphi^{-1} $.
\end{proof}
...
Per far vedere che $ F $ sia un'applicazione liscia tra varietà, dobbiamo verificare che le composizioni $ \varphi_{i} \circ F \circ \id_{\R} = \varphi_{i} \circ F $ siano lisce per $ \forall i=1,\dots,4 $: possiamo scrivere che l'insieme $ \varphi(F^{-1}(V) \cap U) $ della definizione corrisponde a $ \id_{\R}(F^{-1}(U_{i}) \cap \R) = F^{-1}(U_{i}) \subset \R $ per questo caso, da cui le composizioni
...
\begin{proof}
	\`{E} sufficiente verificare che $ (U,F) $ sia compatibile con ogni carta $ (U_{\alpha},\varphi_{\alpha}) $ dell'atlante massimale $ \mathfrak{N} $ di $ N $ che definisce la sua struttura differenziabile, cioè appartiene all'atlante stesso. \\
...
\begin{proof}
	...
	Calcoliamo ora l'azione di $ \varphi_{*p} $ su un vettore generico dell'insieme $ \mathcal{B}_{T_{p}(N)} $ applicato a una qualsiasi funzione $ f \in [(f,U)] \in C_{p}^{\infty}(N) $:
...
\begin{proof}
	...
	per la Proposizione \ref{loc-exp-tan-cur} in quanto $ \alpha $ è anch'essa una curva liscia e possiamo portare $ \varphi_{*0} $ dentro la sommatoria in quanto applicazione lineare.
\end{proof}
...
\begin{itemize}
	...
	\item per un qualunque $ p \in S $ si può definire una carta $ (U \cap S,\varphi_{S}) $ intorno a $ p $ dal quale si ottiene un atlante topologico per $ S $
\end{itemize}
...
\begin{proof}
	Per dimostrare che $ S $ sia una varietà differenziabile, dobbiamo far vedere che per qualsiasi punto $ p \in S $ e qualsiasi coppia di carte $ (U \cap S,\varphi_{S}) $ e $ (V \cap S,\psi_{S}) $ indotte da carte adattate $ (U,\varphi) $ e $ (V,\psi) $ si abbia che $ (U \cap S,\varphi_{S}) $ sia $ C^{\infty} $-compatibile con $ (V \cap S,\psi_{S}) $. \\
...
\begin{proof}
	...
	Sia dunque un punto $ p \in f^{-1}(0) $, vogliamo trovare una carta adattata di $ N $ intorno a $ p $ relativamente a $ f^{-1}(0) $. Il fatto che $ 0 \in \VR_{f} $ implica che almeno una delle derivate parziali di $ f $ non si annulli se consideriamo una qualsiasi carta $ (U,\varphi) $ con $ \varphi = (x^{1},\dots,x^{n}) $ intorno a $ p $. \\
...
dunque $ \varphi $ è un diffeomorfismo: questo significa che $ (\R^{n+1},\varphi) $ è una carta di $ \R^{n+1} $ con la struttura euclidea standard. Questa carta può anche essere pensata come carta adattata di $ \R^{n+1} $ relativamente a $ \Gamma(F) $ per qualsiasi punto $ p \in \Gamma(F) $: questo è vero in quanto l'insieme
...
\begin{equation}
	\begin{cases}
		...
		\pi(v) = p \\ \\
		\varphi_{*}(v) = \displaystyle \sum_{i=1}^{n} a^{i}(v) \eval{ \pdv{r^{i}} }_{p} \longleftrightarrow a(v) = (a^{1}(v),\dots,a^{n}(v)) \in \R^{n}
	\end{cases}	
\end{equation}
...
\begin{equation}
	\begin{cases}
		...
		\pi(v) = p \\ \\
		\varphi_{*}(v) = \displaystyle \sum_{i=1}^{n} b^{i}(v) \eval{ \pdv{r^{i}} }_{p} \longleftrightarrow b(v) = (b^{1}(v),\dots,b^{n}(v)) \in \R^{n}
	\end{cases}	
\end{equation}
