...
Sia $ M $ una varietà differenziabile di dimensione $ m $ e sia la funzione $ f : M \to \R $, diremo che $ f $ è $ C^{\infty} $ (liscia o differenziabile) in un punto $ p \in M $ se esiste una carta $ (U,\phi) $ intorno a $ p $ tale che la funzione $ f \circ \phi^{-1} : \phi(U) \to \R $, con $ \phi(U) \subset \R^{n} $, sia liscia.
...
\begin{remark}
	Se $ f : M \to \R \in C^{\infty} $ allora $ f $ è continua. Infatti $ f_{|\phi(U)} = f \circ \phi^{-1} \circ \phi $ dove $ \phi $ è continua perché omeomorfismo e $ f \circ \phi^{-1} $ è continua in quanto liscia.
\end{remark}

Indicheremo con $ C^{\infty}(M) $ l'insieme delle funzioni $ f : M \to \R \in C^{\infty} $ su $ M $ mentre $ C^{0}(M) $ sono le funzioni continue.

\begin{definition}
	Sia $ M $ una varietà differenziabile con atlante massimale $ \mathfrak{M} $ e sia un atlante differenziabile $ \mathfrak{U} \subset \mathfrak{M} $, allora $ f : M \to \R $ è liscia se e solo se per qualunque carta $ (V,\psi) \in \mathfrak{U} $ si ha che $ f \circ \psi^{-1} : \psi(V) \to \R $ sia liscia.
\end{definition}

\begin{proof}[Dimostrazione ($ \implies $)]
	Se $ f \in C^{\infty}(M) $ allora esiste una carta $ (V,\psi) $ dell'atlante massimale tale che $ f \circ \psi : \psi(V) \to \R $ sia liscia in $ p $ allora lo è anche in $ \psi(V) $ in quanto la scelta non dipende dall'atlante, quindi possiamo prendere la carta dell'atlante $ \mathfrak{U} $.
\end{proof}

\begin{proof}[Dimostrazione ($ \impliedby $)]
	Sia $ p \in M $, allora esiste una carta $ (V,\psi) \ni p $ tale che, per ipotesi, $ f \circ \psi : \psi(V) \to \R $ sia liscia, dunque abbiamo trovato una carta in cui è liscia perciò $ f \in C^{\infty} $ in $ p $.
\end{proof}
...
con $ \phi(F^{-1}(V) \cap U) \in \R^{n} $, sia liscia in $ \phi(p) $\footnote{%
...
\begin{definition}[Composizione di funzioni lisce tra varietà]
	Siano $ F : N \to M $ e $ G : M \to P $ due funzioni continue e lisce e $ M $, $ N $ e $ P $ varietà differenziabili. La composizione continua $ G \circ F $ è liscia.
\end{definition}

\begin{proof}
	Siano $ (U,\phi) $ una carta di $ N $ intorno a $ p \in N $ e $ (W,\sigma) $ una carta di $ P $ intorno a $ G(F(p)) $. Dobbiamo dimostrare che la funzione $ \sigma \circ G \circ F \circ \phi^{-1} $ sia liscia nel punto $ \phi(p) \in (G \circ F)^{-1}(W) \cap U = F^{-1}(G(W)) \cap U $.\\
	...
	la quale è liscia, in quanto composizione di applicazioni lisce (i.e. $ \sigma \circ G \circ \psi $ e $ \psi^{-1} \circ F \circ \phi^{-1} $), in $ \phi(F^{-1}(G(W) \cap V) \cap U) \ni \phi(p) $.
\end{proof}
...
\begin{theorem}
	Una varietà differenziabile di dimensione 1 connessa (curva differenziabili) è diffeomorfa o a $ \S^{1} $ (compatta) o a $ \R $ (non compatta).
\end{theorem}
...
i.e. $ \phi_{\alpha} \circ \pi_{N} \circ (\phi_{\alpha} \times \psi_{\beta})^{-1} \equiv \pi_{M} $, la quale è liscia.\\\\
%
...
Infatti, se $ F $ è liscia allora $ F_{i} = \pi_{i} \circ F $ dove $ \pi_{i} : M_{1} \times M_{2} \to M_{i} $, ma $ \pi_{i} \in C^{\infty} $ per $ i=1,2 $, dunque la composizione che risulta in $ F_{i} $ è liscia.\\
...
è liscia per definizione di applicazione liscia.
...
Per dimostrare che sia un gruppo di Lie, dobbiamo verificare che le operazioni di moltiplicazione e di inversione siano lisce: il prodotto tra matrici
...
\begin{proof}
	...
	per $ \forall i=1,\dots,n $, la quale è la derivata di una composizione di funzioni lisce, i.e. $ f \circ \phi^{-1} $. Deduciamo dunque che la derivata è liscia in quanto l'equazione sopra corrisponde alla definizione di funzione liscia su varietà.
\end{proof}
...
} di funzioni lisce in un intorno di $ p $: un suo elemento è una classe di equivalenza $ [(f,U)] $ dove $ p \in U $ aperto per il quale vale la seguente relazione di equivalenza, i.e. due germi di funzioni appartengono alla stessa classe
