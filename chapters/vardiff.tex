...
\begin{equation}
	f \circ \psi^{-1} = \eval{ (f \circ \phi^{-1})(\phi \circ \psi^{-1}) }_{\psi(U \cap V)} : \psi(U \cap V) \to \R
\end{equation}
...
Dobbiamo verificare che l'applicazione $ \phi_{i} \circ F \circ \id_{\R} $ sia liscia per $ \forall i=1,\dots,4 $: innanzitutto $ \phi(F^{-1}(V) \cap U) $ della definizione corrisponde a $ \id_{\R}(F^{-1}(U_{i}) \cap \R) = F^{-1}(U_{i}) \subset \R $, dunque le composizioni
...
sono tutte lisce perciò lo è anche $ F $.
...
\begin{theorem}
	Una varietà differenziabile di dimensione 1 connessa (curve differenziabili) è diffeomorfa a $ \S^{1} $ (compatta) oppure a $ \R $ (non compatta).
\end{theorem}

\sbs{0.55}{
	\begin{theorem}
		Una varietà differenziabile di dimensione 2 (superficie differenziabile) compatta e connessa è diffeomorfa a $ \Sigma_{g} $, i.e. una superficie con $ g $ \textit{buchi}. La superficie differenziabile $ \Sigma_{g} $ è una generalizzazione del toro (il quale ha un solo buco).
	\end{theorem}
}
{0.45}{
	\img{0.8}{img21}
...
Queste due strutture differenziali sono diverse: le loro carte non sono $ C^{\infty} $-compatibili, in quanto i cambi di carte
...
non sono lisci.\\
Però sono diffeomorfe tramite l'applicazione continua

\map{F}
	{\R}{\R'}
	{x}{x^{3}}

con inversa anch'essa continua

\map{F}
{\R'}{\R}
{x}{x^{\sfrac{1}{3}}}

Per dimostrare che $ F $ e $ F^{-1} $ siano $ C^{\infty} $, controlliamo la composizione tra carte\footnote{%
