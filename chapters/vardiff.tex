...
Il vantaggio di avere una carta su uno spazio topologico è quello di poterlo ricondurre, almeno localmente, ad $ \mathbb{R} $.
...
i.e. la famiglia delle carte è un \textit{ricoprimento}\footnote{%
	???.%
...
\begin{equation}
	GL_{n}(\mathbb{R}) = \{ A \in M_{n}(\mathbb{R}) \, \mid \, \det(A) = 0 \} \subset M_{n}(\mathbb{R}) = \mathbb{R}^{n^{2}}
\end{equation}
...
\paragraph{6. Gruppo delle matrici con determinante unitario}
...
\paragraph{2. Gruppo lineare speciale}
...
\subsubsection{Parallelizabilità}
...
Siano $ F : N \to M $ una funzione liscia (non necessariamente un diffeomorfismo) e due campi di vettori $ X \in \chi(N) $ e $ Y \in \chi(M) $, si dice che $ X $ è $ F $\textit{-related a} $ Y $ se

\begin{equation}
	(F_{*p}X)_{p} = Y_{F(p)}, \qquad p \in N
\end{equation}
