...
Una \textit{categoria} $ \mathfrak{C} $ è definita come una collezione\footnote{%
	...
	Un morfismo è una mappa tra due oggetti che conserva la struttura dell'oggetto sorgente nell'oggetto obbiettivo; ad esempio, può essere pensato come la generalizzazione nella teoria delle categorie delle funzioni nella teoria degli insiemi o delle funzioni continue in topologia.%
...
\begin{gather}
	...
	Mor(V,W) = Hom(V,W) = \{ \text{applicazioni $ \R $-lineari } f : V \to W \} \qcomma \forall V,W \in Ob(\mathfrak{C})
\end{gather}
...
\begin{gather}
	...
	Mor(X,Y) = C^{0}(X,Y) = \{ \text{applicazioni continue } f : X \to Y \} \qcomma \forall X,Y \in Ob(\mathfrak{Top})
\end{gather}
...
\begin{gather}
	Ob(\mathfrak{C}) = \{ \text{collezione di tutte le coppie } (M,p) \, \mid M \text{ varietà differenziabili}, \, p \in M \}\\
		Mor((M,p),(N,q)) = \{ \text{applicazioni lisce } F : N \to M \, \mid \, F(p)=q \} \qcomma \forall (M,p),(N,q) \in Ob(\mathfrak{C})
\end{gather}
