...
\begin{equation}
	\mathfrak{C} = \left\{ (r,s) \st r,s \in \Q \, \wedge \, 0 \notin (r,s) \right\} \cup \left\{ I_{A}(r,s) \st r,s \in \Q^{+} \right\} \cup \left\{ I_{B}(r,s) \st r,s \in \Q^{+} \right\}
\end{equation}
...
Il vettore tangente $ c'(t_{0}) $ è dunque l'immagine, tramite l'applicazione lineare del differenziale $ c_{*t} $, del vettore $ (\sfrac{\operatorname{d}}{\operatorname{dt}}) |_{t_{0}} $, il quale rappresenta la base canonica\footnote{%
	La notazione che indica che un elemento di una base genera uno spazio è la seguente: $ \expval{ \eval{\dv{t}}_{t_{0}} } = T_{t_{0}}(\R) $.%
} di $ T_{t_{0}}(\R) $.
