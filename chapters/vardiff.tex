...
dove $ a \in \R $ e $ \expval{ \eval{ \dv*{r} }_{c(t_{0})} } = T_{c(t_{0})}(\R) $.\\
...
\begin{proof}
	...
	con $ a^{i} \in \R $ per $ i=1,\dots,n $, in quanto $ \expval{ \eval{ \pdv*{x^{i}} }_{c(t_{0})} } = T_{c(t_{0})}(N) $.\\
...
\begin{proof}
	...
	Fissiamo una carta $ (U,\phi) \in N $ con $ \phi = (x^{1},\dots,x^{n}) $, $ p \in U $ e $ \phi(p) = 0 $, la quale fisserà a sua volta una base per lo spazio tangente $ \expval{ \eval{ \pdv*{x^{i}} }_{p} } = T_{p}(N) $. A questo punto possiamo scrivere un vettore dello spazio tangente come
...
\begin{corollary}
	...
	dove $ f_{*p} : T_{p}(N) \to T_{f(p)}(\R) $ e $ \expval{ \eval{ \dv*{r} }_{f(p)} } = T_{f(p)}(\R) $.
\end{corollary}
...
dove $ \expval{ \eval{ \pdv*{r_{ij}} }_{I} } = T_{I}(GL_{n}(\R)) $; a ogni vettore $ X_{I} \in T_{I}(GL_{n}(\R)) $ si può associare il vettore $ X \in M_{n}(\R) $ prendendo semplicemente $ X=[X_{ij}] $ dalla definizione dello spazio $ T_{I}(GL_{n}(\R)) $. Per quanto riguarda lo spazio di arrivo
...
dove $ X_{p} f \in \R $ e $ \expval{ \eval{ \dv*{r} }_{f(p)} } = T_{f(p)}(\R) $, sia dunque la carta $ (U,\phi) \in N $ con $ \phi = (x^{1},\dots,x^{n}) $ intorno a $ p $, allora
...
\begin{theorem}[Rango costante in geometria differenziale]\label{thrm:const-rank}
...
\begin{theorem}[Immersione locale]\label{thrm:loc-imm}
...
\begin{theorem}[Sommersione locale]\label{thrm:loc-sub}
...
\begin{theorem}\label{thrm:smooth-restriction-subman}
...
la quale è ancora liscia per il Teorema \ref{thrm:smooth-restriction-subman}.
...
questo perché $ T_{p}(U) = \der_{p}(C_{p}^{\infty}(U)) $ ma, siccome il concetto di germe di funzione è locale, possiamo scrivere

\begin{equation}
	T_{p}(U) = \der_{p}(C_{p}^{\infty}(U)) = \der_{p}(C_{p}^{\infty}(M)) = T_{p}(M)
\end{equation}

Esiste una bigezione naturale $ \tilde{\phi} $ tra $ T(U) $ e $ \R^{2n} $: prendendo $ v \in T_{p}(M) $ e sapendo che le derivate parziali rispetto a $ x^{i} $ in $ p $ generano lo spazio tangente, i.e.

\begin{equation}
	\expval{ \eval{ \pdv{x^{i}} }_{p} } = T_{p}(M)
\end{equation}
...
il quale risultato può essere identificato con $ c(v) = (c^{1}(v),\dots,c^{n}(v)) \in \R^{n} $.\\
Considerando $ p = \pi(v) $, si può riscrivere $ \tilde{\phi} $ come
...
Da questi risultati, possiamo dunque trarre le seguenti conseguenze:

\begin{itemize}
	\item $ \tilde{\phi} : T(U) \to \phi(U) \times \R^{n} $ è un omeomorfismo;
	
	\item Preso $ V \subset U $, la topologia indotta da $ T(U) $ su $ T(V) $, dove $ T(V) \subset T(V) $, è la stessa della topologia indotta su $ T(V) $ dall'applicazione $ \eval{\tilde{\phi}}_{T(V)} : T(V) \to \phi(V) \times \R^{n} $.
\end{itemize}
...
\begin{proof}
	...
	Per definizione $ T(U_{\alpha}) \in \B_{T(M)} $ in quanto aperto; abbiamo anche che vale la seguente implicazione
	...
	Siano due aperti $ A \subset T(U) $ e $ B \subset T(V) $, i.e. $ A,B \in \B_{T(M)} $, con $ U $ e $ V $ due aperti coordinati (i.e. fanno parte di carte di $ T(U) $ e $ T(V) $ rispettivamente). Dimostriamo che
	...
	dove $ U \cap V $ è ancora un aperto coordinato della carta $ \left( U \cap V,\eval{\phi}_{U \cap V} \right) $. Dobbiamo verificare ora che $ A \cap B \subset T(U \cap V) $ sia aperto in $ T(U \cap V) $ (i.e. che l'intersezione di due elementi della base si possa scrivere come unione di elementi della base) perché $ \B_{T(M)} $ sia effettivamente una base: osserviamo che
...
i quali devono essere lisci per qualsiasi $ \alpha,\beta \in A $, dove i $ T(U_{\alpha \beta}) $ sono aperti di $ \R^{2n} $.\\
...
A questo possiamo riscrivere l'applicazione come
...
Per la funzione $ \tilde{\phi}_{\beta} $ abbiamo
...
Possiamo dunque scrivere i cambi di carte come
...
\begin{equation}
	\pdv{y^{j}}{x^{k}} \, (p) = \pdv{(\phi_{\beta} \circ \phi_{\alpha}^{-1})^{j}}{r^{k}} \, (\phi(p))
\end{equation}
...
\begin{equation}
	(\phi \circ \pi \circ \tilde{\phi}^{-1}) (\phi(p),c^{1},\dots,c^{n}) = (\phi \circ \pi) \left( p, \sum_{i=1}^{n} c^{i} \eval{ \pdv{x^{i}} }_{p} \right) = \phi(p)
\end{equation}
...
\begin{equation}
	D_{p} = \expval{ (X_{1})_{p}, \dots, (X_{r})_{p} }_{\R}
\end{equation}
...
dove $ D_{p} = \expval{ X_{p} }_{\R} $. L'esistenza di una curva integrale che passa per $ p $ è verificata se $ D $ è involutoria, i.e. (per il teorema di Frobenius) $ D $ è completamente integrabile e quindi esiste la curva integrale ricercata.\\
