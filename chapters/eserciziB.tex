...
questo significa che la funzione $ f $ ha un'inversa destra dunque è suriettiva.\\
...
possiamo invocare il teorema della dimensione

\begin{align}
	\begin{split}
		\dim(\Im(f)) + \dim(\ker(f)) &= \dim(T_{p}(N) \times T_{q}(M))\\
		\cancel{ \dim(T_{(p,q)}(N \times M)) } + \dim(\ker(f)) &= \cancel{ \dim(T_{p}(N) \times T_{q}(M)) }\\
		\dim(\ker(f)) &= 0
	\end{split}
\end{align}
